...
基于存储转发传输机制可知，在理想情况下（忽略传播时延，源与目的端系统之间仅有一条链路路径），由于节点之间仅用一条链路相连，因此从源端系统中发送分组仅可以每次发送一个。当分组交换机每次接收一个分组时，是需要将其全部接收完毕后再开始转发（可以与管道中流水相对比），这就产生了一定的时延。
...
每个分组交换机都具有一个**输出缓存（或输出队列）**，用于存储路由器准备输出的分组。既然是缓存，主要是起到缓冲、暂缓整条链路和当前路由器的输出压力的作用。若该链路正忙于传出其他分组，当前分组到达路由器后边必须在输出缓存中等待。

那么由于输出缓存的关系，当前分组在链路上传输就产生了时延，该时延的主要原因是在当前分组在输出缓存中排队等待，那么这种延时被称为**排队时延**。

分组交换机中输出缓存空间的大小是有限的，当缓存空间被分组完全占满时，新到的分组或队列中排队的分组之一会被丢弃，这种情况是**分组丢失（分组丢包）**。

*转发表和路由转发协议*

每个端系统都有一个 IP 地址，当源主机项目的主机发送分组时，该分组的首部包含了目的地的 IP 地址。路由器基于目的地的 IP 地址进行转发，为了明确具体转发到哪条链路上，依据的是**转发表**。每台路由器的转发表都是通过**路由选择协议**进行设置的。
...
频分复用是根据链路的频谱，在连接期间为每条连接分配一个频段。每个连接的频段的宽度，成为这条连接的**带宽**。

时分复用是将时间划分为固定区间的帧，并且每帧又被划分为固定数量的时隙。对每一条连接指定一个时隙，一条电路的传输速率等于帧速率乘以一个时隙的比特数量。**不必关注每帧中被划分了多少时隙，因为对于一条连接来说，传输数据时总是在每帧的固定时隙上携带数据**。

电路交换与分组交换的区别：

* 电路交换预留了链路资源，分组交换则不预留；
* 分组交换提供了更好的宽带共享；
* 分组交换比电路交换更简单、有效，成本更低；
* 分组交换的性能优于电路交换的性能。
